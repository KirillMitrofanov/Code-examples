...
{
	[TestFixture]
	public class RecursiveBinarySearchTreeIntTest
	{
			[Test]
			...
			{
				RecursiveBinarySearchTree<int> tree = new RecursiveBinarySearchTree<int>(items, comparer);
				List<int> result = new List<int>();
			...
			private static IEnumerable<TestCaseData> TestCasesRemove()
			{
				...
				yield return new TestCaseData(new List<int> { 3, 1, 2 }, 1).Returns(true);
				yield return new TestCaseData(new List<int> { 3, 1, 2 }, 3).Returns(true);
				...
				yield return new TestCaseData(new List<int> { 1, 2, 3, 4 }, 5).Returns(false);
				yield return new TestCaseData(new List<int> { 2 }, 6).Returns(false);
				yield return new TestCaseData(new List<int> { 1, 2 }, 0).Returns(false);
